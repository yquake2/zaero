...
CheckArmor(edict_t *ent, vec3_t point, const vec3_t normal, int damage, int te_sparks, int dflags)
{
	...
	{
		save = ceil(((gitem_armor_t *)armor->info)->energy_protection * (double)damage);
	}
	else
	{
		save = ceil(((gitem_armor_t *)armor->info)->normal_protection * (double)damage);
	}
